                            ...
                            <div >
                                ...
                                <div class="">
                                    <div class="card my-3 p-3 surface-50">
                                        <NotificationsListPage ref="notificationsListPage"  :limit="10" field-name="notifications.student_id" :field-value="item.userid" :show-header="true" :show-breadcrumbs="true" :show-footer="true" :paginate="true" page-store-key="INDEX-NOTIFICATIONS" is-sub-page>
                                        </NotificationsListPage>
                                    </div>
                                </div>
                                <div class="">
                                    <div class="card my-3 p-3 surface-50">
                                        <CertificatesListPage ref="certificatesListPage"  :limit="5" :show-header="true" :show-breadcrumbs="true" :show-footer="true" :paginate="true" page-store-key="INDEX-CERTIFICATES" is-sub-page>
                                        </CertificatesListPage>
                                    </div>
                                </div>
                                <div :class="{ 'card ': !isSubPage }" class="">
                                    <div class="q-pa-md">
                                        <div class="font-bold text-lg">course_grade</div>
                                        <div class="text-500"></div>
                                        <hr />
                                        <div class="row q-col-gutter-sm">
                                            <div class="col">
                                                <api-data-source :enable-cache="false"   api-path="components_data/linechart_course_grade" >
                                                    <template v-slot="req">
...
import VueChart from 'src/components/VueCharts.vue';
	...
	function getActionMenuModel(data){
		return [
		{
			...
			icon: "pi pi-pencil",
			visible: auth.canManage('users/edit', data.userid)
		},
		{
			label: () => $t('delete'),
			command: (event) => { deleteItem(data.userid) },
			icon: "pi pi-trash",
			visible: auth.canManage('users/delete', data.userid)
		}
	]
	}
	onMounted(()=>{
